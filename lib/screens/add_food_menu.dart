...
class _AddFoodMenuState extends State<AddFoodMenu> {
        ...
        onPressed: () {
          if (file == null) {
            normalDialog(context,
                'กรุณาเลือกรูปภาพ อาหาร โดยเลือก กล้องถ่ายรูป หรือ คลังรูป');
              ...
              detail == null ||
              detail.isEmpty) {
            normalDialog(context, 'กรุณา กรอกข้อมูลทุกช่อง');
          } else {
  ...
  Future<Null> uploadFoodAndInsertData() async {
    String urlUpdate = '${MyConstant().domain}/TakFood/saveFood.php';
    ...
    try {
      ...
      await Dio().post(urlUpdate, data: fromData).then((value) async {
        String urlPathImage = '/TakFood/Food/$nameFile';

        SharedPreferences preferences = await SharedPreferences.getInstance();
        String idShop = preferences.getString('id');
        String urlInsertData =
            '${MyConstant().domain}/TakFood/addFood.php?isAdd=true&idShop=$idShop&NameFood=$nameFood&PathImage=$urlPathImage&Price=$price&Detail=$detail';
