...
class _EditFoodMenuState extends State<EditFoodMenu> {
    ...
    return FloatingActionButton(
      onPressed: () {
        if (name.isEmpty || price.isEmpty || detail.isEmpty) {
          normalDialog(context, 'กรุณา..กรอกข้อมูลให้ครบ ทุกช่อง');
        } else {
          confirmEdit();
        }
      },
  ...
  Future<Null> confirmEdit() async {
    showDialog(
      ...
      builder: (context) => SimpleDialog(
        title: Text('ต้องการเปลี่ยนแปลง ข้อมูล'),
        children: [
          Row(
            children: [
              FlatButton.icon(
  ...
  Future<Null> editValueOnMySQL() async {
    ...
    String url =
        '${MyConstant().domain}/TakFood/editFoodWhereId.php?isAdd=true&id=$id&NameFood=$name&PathImage=$pathImage&Price=$price&Detail=$detail';
    await Dio().get(url).then((value) {
      if (value.toString() == 'true') {
        Navigator.pop(context);
      } else {
        normalDialog(context, 'กรุณาลองใหม่ มีข้อมูลผิดพลาด');
      }
    });
  }
